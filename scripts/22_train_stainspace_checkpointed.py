"""Milestone 7E / Script 22: stain-space (OD/HED-style) augmentation ERM baseline — checkpointed training.

Trains ResNet18 for 5 epochs on the SAME cached balanced ``train`` subset used by the
plain-ERM 4A baseline, the 7A moderate-augmentation baseline, and the 7D tuned-augmentation
v2 baseline, but with the validated **optical-density (HED-style) stain-space** augmentation
(``StainSpaceAugment``, Milestone 7E-0) applied to the train data only (``id_val`` uses the
deterministic eval transform). Best checkpoint is selected by ``id_val_loss`` ONLY. This is a
*pathology-specific* stain-robustness / domain-generalization baseline for reviewer risk R3
(no *effective* DG / stain-robustness baseline yet); torchvision colour jitter (7A moderate,
7D tuned) did not lift OOD ranking over plain ERM, motivating the stain-space transform.

Strict scope (Milestone 7E training):
  * ``train`` (stain-space-augmented) for training, ``id_val`` (eval transform) for model
    selection / monitoring ONLY.
  * ``ood_val`` is NEVER read in this script (it is used only afterwards, by script 23, for
    development-stage comparison). ``ood_test`` is NEVER read anywhere.
  * Reuses the validated ``StainSpaceAugment`` — NO GroupDRO/CORAL/DANN.
...
configs/experiments/exp07e_stainspace_resnet18.yaml.
Outputs: per-epoch CSV, text report (results/logs/), checkpoints
(results/checkpoints/exp07e_stainspace_resnet18/).
"""
...
def main() -> int:
    ...
    report_path = _resolve(o_cfg["train_report_txt"])
    ...
    emit("=" * 72)
    emit("Milestone 7E - Stain-space (OD/HED-style) augmentation ERM baseline (script 22)")
    ...
    emit(
        f"augmentation={a_cfg['name']}  od_scale=[{a_cfg['od_scale_min']},{a_cfg['od_scale_max']}] "
        f"od_bias=[{a_cfg['od_bias_min']},{a_cfg['od_bias_max']}] "
        f"hflip={a_cfg['horizontal_flip_p']} vflip={a_cfg['vertical_flip_p']} "
        f"rot={a_cfg['rotation_degrees']}"
    )
    ...
    emit("scope: train (stain-space-augmented) + id_val (eval transform) ONLY - ood_val/ood_test untouched.")
    emit("=" * 72)
    ...
    try:
        ...
        # Train uses OD/HED-style stain-space augmentation; id_val uses the deterministic eval transform.
        stain_space_transform = build_stain_space_aug_transform(a_cfg)
        train_ds = BalancedSubsetDataset(train_examples, transform=stain_space_transform)
        ...
        emit(f"train  transform : StainSpaceAugment (train-only OD/HED-style stain-space augmentation)")

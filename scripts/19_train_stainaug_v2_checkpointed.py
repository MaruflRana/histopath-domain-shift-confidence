"""Milestone 7D / Script 19: tuned stain/color augmentation v2 ERM baseline — checkpointed training.

Trains ResNet18 for 5 epochs on the SAME cached balanced ``train`` subset used by the
plain-ERM 4A baseline and the 7A moderate-augmentation baseline, but with a **stronger but
controlled** stain/color augmentation (``StainColorAugmentV2``) applied to the train data
only (``id_val`` uses the deterministic eval transform). Best checkpoint is selected by
``id_val_loss`` ONLY. This is a stronger stain-robustness / domain-generalization baseline
for reviewer risk R3 (no *effective* DG / stain-robustness baseline yet).

Strict scope (Milestone 7D training):
  * ``train`` for training, ``id_val`` for model selection / monitoring ONLY.
  * ``ood_val`` is NEVER read in this script (it is used only afterwards, by script 20, for
    development-stage comparison). ``ood_test`` is NEVER read anywhere.
  * torchvision augmentation only — NO HED/Macenko, NO GroupDRO/CORAL/DANN.
...
configs/experiments/exp07d_stainaug_v2_resnet18.yaml.
Outputs: per-epoch CSV, text report (results/logs/), checkpoints
(results/checkpoints/exp07d_stainaug_v2_resnet18/).
"""
...
from data.stain_transforms import build_stain_color_aug_v2_transform
...
def main() -> int:
    ...
    emit("=" * 72)
    emit("Milestone 7D - Tuned stain/color augmentation v2 ERM baseline (script 19)")
    ...
    emit(
        ...
        f"translate={a_cfg['translate']} scale={a_cfg['scale']} "
        f"cj_p={a_cfg['color_jitter_p']} bright={a_cfg['brightness']} "
        f"contrast={a_cfg['contrast']} sat={a_cfg['saturation']} hue={a_cfg['hue']} "
        f"autocontrast_p={a_cfg['autocontrast_p']}"
    )
    ...
    emit("scope: train (v2-augmented) + id_val (eval transform) ONLY - ood_val/ood_test untouched.")
    emit("=" * 72)
    ...
    try:
        ...
        # Train uses tuned stain/color augmentation v2; id_val uses the deterministic eval transform.
        stain_v2_transform = build_stain_color_aug_v2_transform(a_cfg)
        train_ds = BalancedSubsetDataset(train_examples, transform=stain_v2_transform)
        ...
        emit(f"train  transform : StainColorAugmentV2 (train-only tuned augmentation)")
